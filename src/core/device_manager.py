...
class ADBInput:
    ...
    def _swipe_path_sendevent(self, points: list[tuple[int, int]], window_w: int, window_h: int) -> None:
        ...
        # Helper to add common fields for a touch point
        def touch_point(tx: int, ty: int) -> None:
            if self.has_slot:
                cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_SLOT} 0")
            cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_TRACKING_ID} 0")
            cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_POSITION_X} {tx}")
            cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_POSITION_Y} {ty}")
            if self.has_touch_major:
                cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_TOUCH_MAJOR} 6")
            if self.has_pressure:
                cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_PRESSURE} 50")

        def move_point(tx: int, ty: int) -> None:
            cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_POSITION_X} {tx}")
            cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_POSITION_Y} {ty}")
            if self.has_pressure:
                cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_PRESSURE} 50")
        ...
        touch_point(tx, ty)
        cmds.append(f"sendevent {dev} {self._EV_KEY} {self._BTN_TOUCH} 1")
        cmds.append(f"sendevent {dev} {self._EV_SYN} {self._SYN_MT_REPORT} 0")
        cmds.append(f"sendevent {dev} {self._EV_SYN} {self._SYN_REPORT} 0")

        # MOVE through intermediate + last points
        ...
        # Touch UP
        cmds.append(f"sendevent {dev} {self._EV_ABS} {self._ABS_MT_TRACKING_ID} 4294967295")
        cmds.append(f"sendevent {dev} {self._EV_KEY} {self._BTN_TOUCH} 0")
        cmds.append(f"sendevent {dev} {self._EV_SYN} {self._SYN_MT_REPORT} 0")
        cmds.append(f"sendevent {dev} {self._EV_SYN} {self._SYN_REPORT} 0")

        full_cmd = ";".join(cmds) + "\n"
        self._send_shell(full_cmd)
